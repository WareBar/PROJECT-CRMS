...
#LOGO
uplogo = Image.open("photos\logo.png")
...
c = Entry(login, width=37, bd=0, show="*", font=("Ariel", 10, "bold"), bg="#D9D9D9")
...
#d.place(x=32, y=240)
"""
e = MD.CTkButton(login, text="LOG IN", font=("Corbel", 16,"bold"), width=272, hover_color="Red", fg_color="#61777B",bg_color="White",corner_radius=0,command=lambda:login_btn())
...
#loading and connecting the database into the program
DATABASE = load_workbook("CRMSdb.xlsx")

#ACCESSING & ASSIGNING DIFFERENT SPREADSHEET 
SIGNUP_TABLE = DATABASE["PENDING SIGNUP"]
CREDENTIALS_TABLE = DATABASE["CREDENTIALS"]
MAIN_DATA = DATABASE["MAIN DATA"]
...
def login_btn():
    ...
    if len(pass_code) <= 0 or len(user_id) <= 0:
        messagebox.showerror("DATA", "PLEASE FILL ALL NECESSARY ENTRY")
    else:
        found = False
        for i in range(2, CREDENTIALS_TABLE.max_row):
            if CREDENTIALS_TABLE["A"+str("i")].value == user_id and CREDENTIALS_TABLE["B"+str("i")].value == pass_code:
                found = True
            else:
                1+1
        root_interface = Toplevel()
